...
type MyVenuesViewProps = {
  venues: any[];
  onAddVenue: () => void;
  onEditVenue: (venue: any) => void;
};

export default function MyVenuesView({ venues, onAddVenue, onEditVenue }: MyVenuesViewProps) {
  // const venues = [
  //   {
  ...
  return (
    <>
      <header className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="font-headline-lg text-headline-lg text-on-surface">My Venues</h1>
          <p className="font-body-md text-body-md text-on-surface-variant mt-2">Manage all your listed venues.</p>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-gutter">
        {venues.map((venue, i) => {
          const categoryStyle = getStatusClasses(venue.category);
          ...
          return (
            <React.Fragment key={i}>
              <OwnerVenueCard 
                ...
                dateCreated={new Date(venue.created_at).toLocaleDateString("en-US", {month: "short", day: "numeric", year: "numeric",})}
                imageSrc={venue.images?.length? `${venue.images[0].image}`: ""}
                imageAlt={venue.imageAlt}
                category={venue.category}
                categoryBg={venue.categoryBg}
                categoryText={venue.categoryText}
                status={venue.status}
                statusClasses={getStatusClasses(venue.status)}
